...
#(N, H, W, C) to(N, C, H, W) 张量维度重新变换  通道提前
...
#构建BN层层归一化  维度，需要的归一化层类型，输入格式，输出格式，数值稳定常数
...
#构建激活函数    修正线性单元    Swish）：光滑的激活函数   高斯误差线性单元
...
#center_feature_scale_proj_weight: 权重用于线性变换，形状应该为 (C, D)，其中 D 可以是所需的输出维度。     偏差
#函数对输入 query 进行线性变换。函数应用权重和偏置，从而改变输入特征。使用 sigmoid 激活函数将线性变换的结果映射到 (0, 1) 区间，这对应于特征缩放因子。
...
class DCNv3_pytorch(nn.Module):
    def __init__(
        ...
        """
        DCNv3 Module
        :param channels      #可变形卷积的核大小。
        :param kernel_size  #深度卷积的核大小
        :param stride
        :param pad    #填充大小
        :param dilation #膨胀大小
        :param group
        :param offset_scale  #用于缩放偏移量的参数。
        :param act_layer   #激活
        :param norm_layer   #归一化    否使用中心特征缩放。
        """
        super().__init__()
        if channels % group != 0:
            raise ValueError(
